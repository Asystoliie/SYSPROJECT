      ...
      if(which == SyscallException){
        ...
        switch(type){
          ...
          case SC_SynchGetString:{
            char stg[reg5];
            synchconsole->SynchGetString(stg , reg5);
            copyStringToMachine(4, stg, reg5);
            //free(stg); //weirdly enough this causes a seg fault
            //delete [] stg;
            break;
          }
          case SC_SynchPutInt:{
            char s[11];
            snprintf(s, 10, "%d",reg4);
            //copyStringFromMachine(4, s, 10);
          ...
          case SC_SynchGetInt:{
            ...
            sscanf(stg, "%d",&i);
            machine->WriteMem(reg4, 4, i); // get the char
            break;
          }
